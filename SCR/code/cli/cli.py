...
def assess(input_file, output_file, config_file, logit, pretty, verbose):
    ...
    try:
        ...
        if verbose:
            click.echo(f"[OK] Assessment complete")
            click.echo(f"[OK] Results saved to: {output_file}")
...
def batch(input_file, output_dir, config_file, logit, summary, verbose):
    ...
    try:
        ...
        with click.progressbar(patients, label='Assessing patients') as bar:
            for i, patient_data in enumerate(bar):
                try:
                    ...
                    click.echo(f"\n[WARNING] Error processing patient {i+1}: {e}", err=True)
                    continue
        
        click.echo(f"\n[OK] Processed {len(results_summary)} patients successfully")
        click.echo(f"[OK] Individual assessments saved to: {output_path}")
        ...
        if summary and results_summary:
            ...
            click.echo(f"[OK] Summary saved to: {summary_file}")
...
def validate(config_file, verbose):
    ...
    try:
        ...
        click.echo("[OK] JSON syntax valid")
        click.echo(f"[OK] Version: {config.config.get('version', 'Unknown')}")
        ...
        for section in sections:
            ...
            click.echo(f"[OK] Section '{section}': {count} entries")
...
def template(output_file):
    ...
    click.echo(f"[OK] Template saved to: {output_file}")
