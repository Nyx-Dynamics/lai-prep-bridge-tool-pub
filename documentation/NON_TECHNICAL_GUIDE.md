...
**Duration**: Based on published implementation literature the bridge period is usually 2-8 weeks, depending on circumstances

### LAI-PrEP Implementation Paradox
Long Acting Injectable PrEP options (cabotegravir, lenacapavir) have been shown to be superior in efficacy to oral PrEP in diverse populations across multiple international clinical trials (HPTN 083, HPTN 084, PURPOSE 1, PURPOSE-2).

CAB and LEN require infrequent dosing (every 2 or 6 months, respectively) providing a solution to the primary barrier which has face oral PrEP implementation (Adherence)

Infrequent dosing of LAI-PrEP increases patient safety considerations and potential for development of HIV resistance when considering initiation

Unlike oral PrEP (where patients can start the same day they get their prescription), based on widely available POC HIV testing options, US CDC and WHO guidelines and clinical trial data, LAI-PrEP **cannot** be started immediately. 

This temporal delay, what we refer to as the "bridge period" in LAI-PrEP initiation creates opportunities for patients to:
- Change their mind
